...
const UserHistory = ({ username, history }) => {
                ...
                <div className="relative flex flex-col gap-1 w-full">
                  ...
                  <div className="absolute bottom-0 left-0">
                    <>{historyData.late_fine_amount === 0 ? "" : (
                      <p className="text-red-500">{historyData.late_fine_amount}<span className="text-[12px]">$</span></p>
                    )}</>
                  </div>
...
export const TimeCounter = ({ acceptedTime, returnTime }) => {
  ...
  const formatTime = (timeUnit) => {
    return timeUnit < 10 && timeUnit >= 0 ? `0${timeUnit}` : timeUnit.toString().startsWith('-') && Math.abs(timeUnit) < 10 ? `${timeUnit.toString().slice(0, 1)}0${timeUnit.toString().slice(1)}` : timeUnit.toString();
  };

  const textColorClass = isNegative ? 'text-red-500' : '';

  return (
    <span className={textColorClass}>{`${timeLeft.days} :  ${formatTime(timeLeft.hours)} : ${formatTime(
      timeLeft.minutes
